...
class SourceImage(TiledImage):
    ...
    def __init__(self, source_path, tile_path = None, bgcolor = 0xFFFFFF):
        TiledImage.__init__(self)
        self.source_path = source_path
        if tile_path == None:
            tile_path = self.defaultTilePath(source_path)
        self.setTilePath(tile_path)
        self.bgcolor = bgcolor
        
    def defaultTilePath(self, source_path):
        """Create a tile path from the source path"""
        (directory, file_name) = os.path.split(source_path)
        (name, ext) = os.path.splitext(file_name)
        return os.path.join(directory, "_images", name)
