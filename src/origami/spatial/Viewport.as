...
package origami.spatial
{
	...
	public class Viewport implements IEventDispatcher
	{
		...
		private function initializeCoordinates(): void
		{
			...
			// Set the scale to the minimum scale (fully zoomed out)
			scale = minimum_scale;
		...
		private function setMinimumScale(): void
		{
			...
			scene_size = view_size.scale(1/ minimum_scale);
		}
		...
		private function update(): void
		{
			...
			scroll_limits = initial_scene_size.rectangle.inset(half_extent.width, half_extent.height);
		...
		public function boxScale(box: Rectangle): Number
		{
			...
			var vertical_scale: Number    = view_size.height * scale / box.height; 	
			var scale: Number = Math.min(horizontal_scale, vertical_scale);
